...
#   ...answer...
#
# These expressions provide a reliable parser for that exact format.
_STUDENT_QUESTION_HEADING_RE = re.compile(
    r"\\(?:sub)*section\*?"
    r"\{\s*Question\s*(\d+)\s*\}",
    re.IGNORECASE,
)

_STUDENT_PART_HEADING_RE = re.compile(
    r"\\textbf"
    r"\{\s*\(([^(){}]+)\)\s*\}",
    re.IGNORECASE,
)

_STUDENT_COMMENT_LINE_RE = re.compile(
    r"(?m)^[ \t]*%.*(?:\n|$)"
)
...
def _clean_student_answer_fragment(
    value: Any,
) -> str:
    """
    Remove wrapper-only noise while preserving the student's LaTeX and
    Hebrew text.
    """
    ...
    # Remove comments generated by OCR/export tools, for example:
    # % WARNING: MathGrade could not confidently detect...
    text = _STUDENT_COMMENT_LINE_RE.sub(
        "",
        text,
    )

    text = _strip_tex_document_wrappers(
        text
    )
...
def _is_substantive_student_answer(
    value: Any,
) -> bool:
    """
    Reject empty fragments and parser artifacts such as bare comments,
    braces, or spacing commands.
    """
    text = _clean_student_answer_fragment(
        value
    )
    ...
    probe = re.sub(
        r"\\(?:par|smallskip|medskip|bigskip|"
        r"vspace|hspace)\b"
        r"(?:\{[^{}]*\})?",
        "",
        text,
...
def _parse_structured_student_tex_answers(
    student_path: Path,
) -> dict[Key, str]:
    r"""
    Parse the OCR review format directly:
    ...
    Questions without a part heading are returned with an empty part.
    They may later be mapped to a sole reference part.
    """
    ...
    question_matches = list(
        _STUDENT_QUESTION_HEADING_RE.finditer(
            source
        )
    )
    ...
    for question_index, question_match in enumerate(
        question_matches
    ):
        qnum = int(
        ...
        question_chunk = source[
            question_start:question_end
        ]

        question_chunk = (
            question_chunk.replace(
                r"\end{document}",
                "",
            )
        )
        ...
        part_matches = list(
            _STUDENT_PART_HEADING_RE.finditer(
                question_chunk
            )
        )

        # Questions 4 and 5 in the supplied OCR file do not have an
        # explicit \textbf{(a)} marker.
        if not part_matches:
            answer = (
        ...
        for part_index, part_match in enumerate(
            part_matches
        ):
            raw_part = (
                part_match.group(1)
            ...
            if not _is_substantive_student_answer(
                answer
            ):
                continue
...
def _merge_student_answer_maps(
    parsed_answers: dict[Key, str],
    structured_answers: dict[Key, str],
) -> dict[Key, str]:
    """
    Prefer explicit OCR question/part headings over guesses made by the
    legacy TeX parser.
    """
    merged: dict[Key, str] = {}
    ...
    for key, value in parsed_answers.items():
        ...
        if _is_substantive_student_answer(
            ...
            merged[normalized_key] = cleaned

    # Remove an incorrectly parsed global Q1 when explicit Q1(a), Q1(b),
    # etc. were found by the structured parser.
    structured_questions_with_parts = {
        qnum
        for qnum, part in structured_answers
        if part
    }

    for qnum in structured_questions_with_parts:
        merged.pop(
            _normalize_key(
                (qnum, "")
            ),
    ...
    # Explicit structured headings take precedence.
    for key, value in structured_answers.items():
        normalized_key = _normalize_key(
            key
        )
...
def _align_student_answers_to_reference(
    ...
    reference_by_question: dict[
        int,
        list[Key],
    ] = {}

    for key in normalized_reference_keys:
        reference_by_question.setdefault(
            key[0],
            [],
        ).append(key)
    ...
    for raw_key, value in student_answers.items():
        ...
        target_key: Key | None = None

        if key in normalized_reference_keys:
            target_key = key

        elif not key[1]:
            candidates = (
                reference_by_question.get(
                    key[0],
                    [],
                )
            )
...
def _load_student_answers_for_payloads(
    student_path: Path,
    out_dir: Path,
) -> tuple[
    dict[Key, str],
    ...
    # it to block the more reliable OCR-heading parser.
    try:
        (
            student_answers_raw,
            _student_ranges,
        ) = parse_student_tex_answers(
            student_path,
            out_dir,
        )
    except Exception:
    ...
    parsed_answers = {
        ...
        for key, value
        in student_answers_raw.items()
    }

    structured_answers = (
        _parse_structured_student_tex_answers(
            student_path
        )
    )

    merged = _merge_student_answer_maps(
        parsed_answers,
        structured_answers,
    )
...
def _build_payloads_from_full_solution_json(
    *,
    ...
    if not parsed_student_answers:
        raise RuntimeError(
            "Could not parse any student answers from the uploaded "
            "student file. Check debug_student_tex_parts.txt and "
            "debug_student_answer_alignment.json."
        )
    ...
    _write_student_alignment_debug(
        out_dir=out_dir,
        parsed_answers=(
            parsed_student_answers
        ),
    ...
    #
    # The previous union added every unanswered reference question to
    # grades.json, producing many false 'no submission' records.
    keys = sorted(
    ...
    manifest = {
        ...
        "subject": next(
            (
                ...
                for key in ref_parts.keys()
            ),
            "math",
        ),
        "grading_prompt_extra": next(
            (
                str(
                    (
                            ref_parts.get(key)
